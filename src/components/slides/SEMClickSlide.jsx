...
function SEMClickSlide() {
  ...
  const summarizeData = () => {
    ...
    const overallText = `Overall conversions: ** ${parseFloat(
      SEMTotals?.Clicks?.toFixed(2) || 0
    )?.toLocaleString()} clicks**`;
    summaryTextTemp = `<li>${overallText}</li>`;

    const averageCPC = SEMTotals?.cpc;

    summaryTextTemp += `<li>Average CPC: **${averageCPC?.toLocaleString(
      undefined,
    ...
    if (topConversionChannel[0] != topConversionRateChannel[0]) {
      ...
      } with ${topConversionRateChannel[1].ctr.toFixed(2)}% of %CTR**`;
